...
| PAST-POST-04 | corpo vazio (`{}`) | — | 400 | `field` = `body`; `message` = "Forneça os dados da pastagem." |
...
| PAST-POST-11 | `propriedadeId` inexistente | — | 404 | `errorType` = `resourceNotFound`; `field` = `Propriedade` |
...
| PAST-POST-13 | `propriedadeId` aponta para propriedade inativa | propriedade de A com `ativo: false` | 400 | `errorType` = `validationError`; `field` = `propriedadeId`; `message` = "Propriedade está inativa." |
| PAST-POST-14 | nome duplicado: já existe pasto **ativo** com o mesmo nome na mesma propriedade | A tem pasto "Piquete 1" ativo nessa propriedade | 409 | `errorType` = `conflict`; `field` = `nome` |
...
| PAST-POST-17 | sem token | — | 401 | `errorType` = `unauthorized` |
...
| PAST-GET-14 | sem token | — | 401 | `errorType` = `unauthorized` |
...
| PAST-GET-ID-03 | id inexistente | — | 404 | `errorType` = `resourceNotFound`; `field` = `Pastagem` |
...
| PAST-GET-ID-06 | sem token | — | 401 | `errorType` = `unauthorized` |
...
| PAST-PATCH-ID-04 | corpo vazio (`{}`) | — | 400 | `message` = "Forneça pelo menos um campo para atualizar." |
...
| PAST-PATCH-ID-07 | id inexistente | — | 404 | `errorType` = `resourceNotFound` |
| PAST-PATCH-ID-08 | multi-tenancy: B tenta editar pasto de A | — | 404 | mesma resposta do cenário anterior |
| PAST-PATCH-ID-09 | `nome` já usado por outro pasto ativo na mesma propriedade | — | 409 | `errorType` = `conflict`; `field` = `nome` |
| PAST-PATCH-ID-10 | `status: "Vazio"` com rebanho ativo alocado no pasto | pasto tem rebanho ativo (`pastoAtualId`) | 400 | `errorType` = `validationError`; `field` = `status`; `message` inclui "há rebanhos no pasto" |
| PAST-PATCH-ID-11 | `status: "Descanso"` com rebanho ativo alocado | idem | 400 | mesma trava do cenário anterior |
| PAST-PATCH-ID-12 | `ativo: false` com rebanho ativo no pasto | idem | 400 | `errorType` = `validationError`; `field` = `ativo`; `message` = "Pasto ainda contém rebanhos vinculados." |
...
| PAST-PATCH-ID-16 | sem token | — | 401 | `errorType` = `unauthorized` |
...
| PAST-DELETE-ID-02 | recusa exclusão com rebanho ativo no pasto | — | 400 | `errorType` = `validationError`; `field` = `ativo` |
| PAST-DELETE-ID-03 | id inexistente | — | 404 | `errorType` = `resourceNotFound` |
...
| PAST-DELETE-ID-06 | sem token | — | 401 | `errorType` = `unauthorized` |

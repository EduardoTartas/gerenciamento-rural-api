...
| MOV-POST-03 | corpo vazio | — | 400 | `field: body` |
| MOV-POST-04 | campo extra no corpo (`.strict()`) | — | 400 | `errorType: validationError` |
...
| MOV-POST-08 | sem token | — | 401 | `errorType: unauthorized` |
| MOV-POST-09 | admin (não dono) tenta mover rebanho de A | token admin | 404 | `field: Rebanho`; sem bypass |
| MOV-POST-10 | B tenta mover rebanho de A (`rebanhoId` de A) | — | 404 | `field: Rebanho` |
| MOV-POST-11 | `rebanhoId` inexistente | — | 404 | `field: Rebanho` |
| MOV-POST-12 | rebanho inativo | rebanho de A com `ativo:false` | 400 | `field: rebanhoId`, mensagem "Rebanho está inativo" |
| MOV-POST-13 | `pastoDestinoId` inexistente | — | 404 | `field: Pastagem` |
| MOV-POST-14 | `pastoDestinoId` pertence a B | B com pasto próprio, A autenticado | 404 | `field: Pastagem` |
| MOV-POST-15 | pasto de destino inativo | — | 400 | `field: pastoDestinoId`, mensagem "Pasto de destino está inativo" |
| MOV-POST-16 | destino igual ao pasto atual do rebanho | `pastoDestinoId` = `pastoOrigemA` | 400 | `field: pastoDestinoId`, mensagem "já está neste pasto" |
| MOV-POST-17 | destino de propriedade diferente da do rebanho | — | 400 | `field: pastoDestinoId`, mensagem "não pertence à mesma propriedade" |
| MOV-POST-18 | destino ocupado por outro rebanho ativo, sem `permitirLotacaoConjunta` | 1 rebanho ativo no destino | 400 | `field: pastoDestinoId`, mensagem "já tem outro lote" |
...
| MOV-GET-14 | query com campo extra | — | 400 | `errorType: validationError` |
| MOV-GET-15 | sem token | — | 401 | `errorType: unauthorized` |
...
| MOV-GET-ID-03 | id inexistente | — | 404 | `field: Movimentação` |
| MOV-GET-ID-04 | sem token | — | 401 | `errorType: unauthorized` |
...
| MOV-DELETE-ID-02 | tenta desfazer uma movimentação que não é a última | rebanho tem 2+ movimentações, alvo não é a mais recente | 409 | `errorType: conflict`, `field: id`, mensagem cita o id da última válida |
...
| MOV-DELETE-ID-08 | id inexistente | — | 404 | `field: Movimentação` |
| MOV-DELETE-ID-09 | sem token | — | 401 | `errorType: unauthorized` |
| MOV-DELETE-ID-10 | multi-tenancy: B tenta desfazer movimentação de A | — | 404 | `field: Movimentação` |

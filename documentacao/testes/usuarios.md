...
| USR-GET-06 | campo de query não reconhecido (`.strict()`) | admin; `?foo=bar` | 400 | `errorType` `validationError` |
...
| USR-GET-ID-04 | ID em formato inválido (não UUID) | A; `/usuarios/abc` | 400 | `errorType` `validationError` (Zod, via `UserIdSchema`) |
...
| USR-PATCH-06 | campo extra no corpo (`.strict()`) | A; `id` = A; `{ name: "X", admin: true }` | 400 | `errorType` `validationError` — confirma que `admin` não pode ser autopromovido via este endpoint |
...
| USR-PATCH-12 | ID em formato inválido | A; `/usuarios/abc` | 400 | `errorType` `validationError` |
...
| USR-PATCH-FOTO-06 | campo extra no corpo (`.strict()`) | A; `id` = A; `{ url: "...", nome: "x" }` | 400 | `errorType` `validationError` |
...
| USR-PATCH-FOTO-12 | ID em formato inválido | A; `/usuarios/abc/foto` | 400 | `errorType` `validationError` |
...
| USR-DELETE-06 | ID em formato inválido | A; `/usuarios/abc` | 400 | `errorType` `validationError` |
